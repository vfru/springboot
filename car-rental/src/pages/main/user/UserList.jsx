...
export default function UserList() {
  ...
  const { role, roleId, username, isDefault } = JSON.parse(localStorage.getItem('token'))
  // 添加用户功能开关
  ...
  useEffect(() => {
    ...
    axios.get("users/roles").then(res => {
      ...
      // console.log(list)
      // 筛选出经理和销售,
      setdataSource(roleId === 1 && isDefault ? list : [
        // 筛选出自己，
  ...
  const list = role.rights.filter(data => Object.keys(buttonList).some(i => data === i))
  //表格的标题，框架
  const columns = [
    {
      title: '用户昵称',
      ...
      render: (item) => {
        ...
        return <div>
...

          {
            ...
            item.block === false && buttonList[list[1]] === "拉黑" && <Button onClick={() => { blockUser(item, true) }} type='primary' danger disabled={item.username === username ? true : false} >拉黑</Button>
          }
          {
            // 
            item.block === true && buttonList[list[1]] === "拉黑" && <Button onClick={() => { blockUser(item, false) }} type='primary' disabled={item.username === username ? true : false} >恢复</Button>
          }
          &nbsp;
          {
            // 自己不能删除自己
            buttonList[list[3]] === "删除" && <Button onClick={() => { toConfirm(item) }} type='primary' danger disabled={item.username === username ? true : false} >删除</Button>
          }
        </div>
      }
    }
  ...
  const updateUser = (item) => {
    console.log(item)
    if ((item.id === 1 && item.isDefault) || roleId >= 2) {
      setisdis(true)
    } else {
      setisdis(false)
    }
  ...
  return (
    <div>
      ...
      <Modal
        ...
        onOk={addUser}
      >
        <UserForm ref={userData} roleList={roleList} isDefault={isDefault} />
      </Modal>
      ...
      <Modal
        ...
        onOk={updateOK}
      >
        <UserForm ref={userData} roleList={roleList} isDefault={isDefault} isdis={isdis} />
      </Modal>
    </div>
  )
}
      ...
      >
        ...
        >
          <Select disabled={props.isdis} >
            {/* isDefault判断是否为总管理员，总管理员可以创造其他管理员，其他只能创建销售和客户 */}
            {
              props.isDefault ? props.roleList.map(item =>
                <Option key={item.id} value={item.id} >{item.roleName}</Option>)
                :
                props.roleList.map(item =>
                  <Option key={item.id} value={item.id} disabled={item.id === 1} >{item.roleName}</Option>)
            }
          </Select>
        </Form.Item>
      </Form>
