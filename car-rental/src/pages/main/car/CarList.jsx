...
function CarList(props) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (state) => {
                if (state === 0) {
                    return <div style={{color: "grey"}}>{carState[state]}</div>
                }
                if (state === 1) {
                    return <div style={{color: "black"}}>{carState[state]}</div>
                }
                if (state === 2) {
                    return <div style={{color: "green"}}>{carState[state]}</div>
                }
                if (state === 3) {
                    return <div style={{color: 'red'}}>{carState[state]}</div>
                }
                if (state === 4) {
                    return <div style={{color: "skyblue"}}>{carState[state]}</div>
                }
            }
        ...
        },
        {
            ...
            render: (item) => {
                ...
                if (roleId === 3 && (carState[item.state] === "待出租" || carState[item.state] === "优惠中")) {
                    ...
                    return <div>
                        {
                            carState[item.state] === "维修中" &&
                            <Button type='primary' onClick={() => changeCarState(1, item)}>上架</Button>
                        }&nbsp;&nbsp;
                        {
                        ...
                        {
                            (carState[item.state] === "待出租" || carState[item.state] === "优惠中") &&
                            <Button type='primary' danger onClick={() => changeCarState(0, item)}>下架</Button>
                        }&nbsp;&nbsp;
                        {
    ...
    return (
        <div>
            ...
            {/* 添加按钮 */}
            {(roleId === 1 || roleId === 0) && <Button type='primary' onClick={() => {
                setrevampCar(true);
                setnewACar(true)
            }}>新增车辆</Button>}
            {/* 修改添加功能 */}
            {revampCar &&
                <RevampCar settoUpdate={settoUpdate} newACar={newACar} setnewACar={setnewACar}
                           setrevampCar={setrevampCar} cardetails={cardetails} ref={carDataref} salesList={salesList}
            ...
            >
                <div>
                    <Descriptions title="租赁清单" bordered column={3}>
                        ...
                        <Descriptions.Item label="汽车图片" span={1}><img style={{height: "150px", width: "240px"}}
                                                                      src={cardetails.img}
                                                                      alt="图片"/></Descriptions.Item>
                        ...
                        <Descriptions.Item label="出行保障套餐" span={1}>
                            <Radio.Group onChange={onSelectChange} value={selectValue}>
                                <div>
                                    {
                                        insuranceList.map(
                                            item => <Radio key={item.id} value={item.value} details={item.details}
                                                           price={item.price} id={item.id}>{item.label}</Radio>
                                        )
                                    }
                                </div>
                            </Radio.Group>
